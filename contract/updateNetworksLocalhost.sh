...
REGISTRATION_ADDRESS=`jq -r '."BattleRoyale#RegistrationPunk"' ./ignition/deployments/chain-1337/deployed_addresses.json`
REGISTRATION_ADDRESS="$REGISTRATION_ADDRESS" jq '. | .localhost.Registration.address = env.REGISTRATION_ADDRESS' ../graph/newSubgraph/networks.json > ../graph/newSubgraph/networks.tmp
mv ../graph/newSubgraph/networks.tmp ../graph/newSubgraph/networks.json
echo "Registration address updated in networks.json to `jq -r '.localhost.Registration.address' ../graph/newSubgraph/networks.json`"
...
jq '.abi' ignition/deployments/chain-1337/artifacts/BattleRoyale\#RegistrationPunk.json > ../graph/newSubgraph/abis/Registration.json
