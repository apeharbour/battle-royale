...
import MapAbi from './abis/MapNP.json'
import GameAbi from './abis/GameNP.json'
...
function GameMode3() {
  ...
  const [maxAllowedDistance, setMaxAllowedDistance] = useState(0)
  ...
  const addShip = async (speed, range) => {
    ...
    fetchShips()
    setMaxAllowedDistance(Math.ceil(speed/10))
  }
  ...
  const addShipWoYacht = async () => {
    ...
    console.log('Added ship')
    fetchShips()
    setMaxAllowedDistance(Math.ceil(50/10))
  }
  ...
  return (
    <Container>
      <Grid container spacing={4} mt={4}>
        <Grid item xs={12}>
          <Typography variant='h3'>Battle Royale - with Yacht and new probability</Typography>
        </Grid>
            ...
            <Stack spacing={4}>
              ...
              <Stack spacing={2} direction='column'>
                <TextField
                  ...
                  label='Move Distance'
                  type='number'
                  inputProps={{ min: '0', max: `${maxAllowedDistance}`, step: '1' }}
                  onChange={event => {
                    let newValue = event.target.value
                    if (newValue === '' || newValue < 0) {
                      newValue = '0'
                    } else if (newValue > maxAllowedDistance) {
                        newValue = maxAllowedDistance.toString()
                    }
                ...
                <Typography variant='body2'>
                    Max allowed distance: {maxAllowedDistance}
                </Typography>
                <TextField
                  required
                  id='outlined-required'
